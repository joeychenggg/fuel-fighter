...
# OLD CODE
def first_pix(op): 
    y_coordinate = op(STARTING_POINT[0], 1)
    inArray = binarySearch(arr, 0, len(arr)-1, y_coordinate) # THE INDEX i IS UNECESSARY HERE
    while True:
        if inArray: 
...
def left_line(op):
    ...
    inArray, indexFromBinarySearch = binarySearch(arr, 0, len(arr)-1, x_coordinate)
    ...
    for i in range(3):
        if inArray: 
            ...
            inArray, indexFromBinarySearch = binarySearch(arr, indexFromBinarySearch, indexFromBinarySearch + 101, x_coordinate)
        else:
            x_coordinate = op(x_coordinate, 100) 
            if count != 0:
                inArray = binarySearch(arr, indexFromBinarySearch, indexFromBinarySearch + 1000, x_coordinate)
            else:
...
def run():
    ...
    print(f"ALL GREY PIX: {ALL_GREY_PIX}") 
